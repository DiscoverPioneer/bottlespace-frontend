...
function ImageGallery() {
  ...
  const submitBusinessPhoto = async (e) => {
    ...
    //UploadBusinessPhoto(currentUser.establishments[0].id, form)
    //  .then((response) => {
  ...
  useEffect(() => {
    ...
    GetPresignedURL(currentUser.establishments[0].id)
      .then((resp) => {
        ...
        setSignedUrl(u);
        let url = new URL(u);
        setBucket(resp.data.bucket);
        setKey(resp.data.key);
        setAwsAccessKey(url.searchParams.get("AWSAccessKeyId"));
        setSignature(url.searchParams.get("Signature"));
      }).catch((err) => {
  ...
  return (
    <div className="ImageGallery">
      <form action={signedUrl} method="post" enctype="multipart/form-data">
        <input
          id="file"
          ref={fileReference}
          type="file"
